...
class DayCell(QFrame):
    def __init__(self, day: date, current_month: int):
        super().__init__()
        ...
        if day.month != current_month:
            self.setStyleSheet("""
                ...
                QFrame QLabel { color: rgba(255,255,255,0.45); }
            """)
        else:
            self.setStyleSheet("""
                QFrame {
                ...
                QFrame QLabel { color: rgba(255,255,255,0.92); }
            """)
...
class YearCalendarWidget(QWidget):
    ...
    def _build_months(self):
        ...
        for row in range(3):
            for col in range(4):
                self._grid.addWidget(MonthWidget(self.year, month), row, col)
                month += 1
